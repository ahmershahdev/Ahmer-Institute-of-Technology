...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['challan_pic'])) {
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: " . (!empty($_SESSION['student_code']) ? 'student-dashboard' : 'dashboard') . "?status=success");
            exit();
        } else {
            $conn->rollback();
    ...
    header("Location: " . (!empty($_SESSION['student_code']) ? 'student-dashboard' : 'dashboard'));
    exit();
}
